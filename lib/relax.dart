...
class _RelaxScreenState extends State<RelaxScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadUserData() async {
    try {
      ...
      // Load the local image path from SharedPreferences
      final String? storedImagePath = prefs.getString('profileImagePath');
      if (storedImagePath != null && await File(storedImagePath).exists()) {
        _profileImage = File(storedImagePath);
      }
      ...
      if (userId != null) {
        ...
        // Update MeditationScreen with default values if no userId
        setState(() {
          _screens[0] = MeditationScreen(
            userName: _userName,
            profileImage: _profileImage,
  ...
  Future<void> _pickImage() async {
    ...
    if (image != null) {
      ...
      if (userId != null) {
        try {
          await uploadProfileImage(userId, _profileImage!);
          // Refresh user data to get updated profileImageUrl
          await _loadUserData();
        } catch (e) {
